...
- **`Run.Text` 绑定一律加 `Mode=OneWay`**。WPF 的 `Run.Text` 依赖属性默认 **TwoWay**，绑到只读属性（record 的 `init` 属性、`=>` 计算属性等）会在该 `Run` 实例化时抛 `InvalidOperationException`。展示用 `<Run Text="{Binding X}">` 一律写 `Mode=OneWay`。
- **新增整页（View）三件套，缺一不可**：① `App.xaml.cs` DI 里 `services.AddTransient<XxxViewModel>()`；② `App.xaml` 资源里加 `<DataTemplate DataType="{x:Type vm:XxxViewModel}"><views:XxxView/></DataTemplate>`；③ `MainViewModel.BuildNav()` 里按角色 `items.Add(new NavItem("标题","key",() => _sp.GetRequiredService<XxxViewModel>()))`。导航靠 VM 类型 → DataTemplate 解析出 View，三处必须同步。
- **弹窗模式**：`new XxxWindow(...) { Owner = Application.Current?.MainWindow }`，构造函数传 `IApiClient` 与所需参数；窗口要么自己调接口并 `DialogResult = true`，要么暴露 `BuildXxxRequest()` / `Built` 供 VM 调接口。VM 命令里 `if (dlg.ShowDialog() == true) await ReloadAsync();`。
- **列表行的条件展示/操作可见性**：把 DTO 包一层 RowViewModel（如 `RoomRowViewModel`/`AppointmentRowViewModel`/`MemberCardRowViewModel`），把"状态中文化、按状态算 Can 操作、可用性"算到 VM 里，避免在 XAML 里堆转换器或多重 DataTrigger。命令 `CommandParameter` 传 `{Binding Dto}`。
- **无障碍**：每个交互控件（`TextBox`/`ComboBox`/`DatePicker`/`CheckBox`/`ListBox`/`DataGrid`/`Button`）必须设 `AutomationProperties.Name`（值取其旁边的标签文案）；金额相关再配 `AutomationProperties.HelpText`。详见「盲人无障碍约定 · CS 端」。
